...
def can_change_friend_request_status(user_id: int, friend_request: DBFriendRequest, cancellation: bool = False) -> bool:
    if cancellation:
...
def validate_can_change_user_membership_role(requesting_user_id: int, user_id: int, new_role: GroupRole, group: DBGroup, db: Session) -> None:
    from db.group_member import get_group_member_role
    ...
    if (
        ...
        and requesting_user_id != group.owner_id
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
def get_user_post_visibility_filter(requesting_user_id: int, user_id: int, are_friends: bool) -> ColumnElement[bool]:
    if requesting_user_id == user_id:
        return true()
...
def can_see_post(requesting_user_id: int, post: DBPost, db: Session) -> bool:
    if not post.group_id:
        ...
        if DBPost.visibility == PostVisibility.friends_only and is_friend_with(db=db, user_id=post.user_id, current_user_id=requesting_user_id):
            return True
...
def can_edit_post(user_id: int, post: DBPost, group_id: int | None = None, db: Session | None = None) -> bool:
    if not group_id:
        ...
        from db.group_member import get_group_member_role
        return True if post.user_id == user_id and post.group_id == group_id and get_group_member_role(db=db, group_id=group_id, user_id=user_id) else False


def can_delete_post(user_id: int, post: DBPost, group_id: int | None = None, db: Session | None = None) -> bool:
    if not group_id:
        return True if post.user_id == user_id else False
    else:
        if not db:
        ...
        print(f"condition1: {condition1}\tcondition2_a: {condition2_a}\tcondition2_b: {condition2_b}")
        print(f"result is {True if condition1 and (condition2_a or condition2_b) else False}")
