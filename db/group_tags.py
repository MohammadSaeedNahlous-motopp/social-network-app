...
def add_tags(tags: list[int], group_id: int, current_user_id: int, db: Session) -> None:
    if not can_edit_group(user_id=current_user_id, group_id=group_id, db=db):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="User can't edit this group"
        )
    ...
    if len(tags) == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Tag list is empty"
        )
    ...
    if len(tags_to_add) == 0:
        raise HTTPException(
            ...
            detail="These tags are already associated with a group",
        )
...
def remove_tags(tags: list[int], group_id: int, current_user_id: int, db: Session):
    ...
    tags_to_remove = (
        tags & existing_tags
    )  # tags that we requested to remove and that are associated with a group

    if len(tags_to_remove) == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="These tags are already not associated with a group",
        )
